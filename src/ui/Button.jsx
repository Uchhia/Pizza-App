...
function Button({ children, disabled, to, type, onClick }) {
  const base = `
  text-sm
  inline-block rounded-full bg-yellow-400 
  ...
  const style = {
    ...
    border-stone-300  uppercase
    font-semibold tracking-wide text-stone-400
   transition-colors duration-300 hover:bg-stone-300
   focus:bg-stone-300 focus:outline-none focus:ring
   hover:ring-stone-800 focus:ring-stone-300
   hover:text-stone-800
   focus:ring-offset-2
   focus:text-stone-800
   disabled:cursor-not-allowed px-4 py-2.5 md:px-6 md:py-3. 
   `,
  };
  if (to)
    return (
